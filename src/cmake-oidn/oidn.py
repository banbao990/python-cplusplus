...
class OidDenoiser:
    def __init__(self) -> None:
        ...
        weights_files_names = [i.split(".")[0].split("_")[-1] for i in weights_files]
    ...
    def render_ui(self, use_albedo_and_normal) -> (bool, bool):
        ...
        # weight
        vc, self.oidn_weight_index = imgui.combo("Weights", self.oidn_weight_index, self.weights_files_names)
        if vc:
            weight_path = ""
        ...
        # aux
        if self.oidn_weight_index == 0:
            vc, use_albedo_and_normal = imgui.checkbox("Use Albedo and Normal", use_albedo_and_normal)
            value_changed = value_changed or vc
        else:
            # weight file indicates whether to use albedo and normal
            tmp = use_albedo_and_normal
            use_albedo_and_normal = self.weights_files[self.oidn_weight_index - 1].find("alb_nrm") != -1
